...
class VNCalendar:
    ...
    # Tết Nguyên Đán dates (Gregorian)
    TET_DATES = {
    ...
    VN_HOLIDAYS = {
        2024: [
            ...
            date(2024, 2, 10), date(2024, 2, 11),
            date(2024, 2, 12), date(2024, 2, 13), date(2024, 2, 14),
            date(2024, 4, 18),
            date(2024, 4, 30), date(2024, 5, 1), date(2024, 9, 2),
        ],
        2025: [
            date(2025, 1, 1),
    ...
    def get_tet_date(self, year: int) -> Optional[date]:
        return self.TET_DATES.get(year)

    def get_tet_window(self, year: int, days_before: int = 15, days_after: int = 7) -> tuple:
        tet = self.get_tet_date(year)
        if tet is None:
            return (date(year, 1, 1), date(year, 12, 31))
        return (tet - timedelta(days=days_before), tet + timedelta(days=days_after))

    def is_in_tet_window(self, check_date: date, days_before: int = 15) -> bool:
        start, end = self.get_tet_window(check_date.year, days_before)
        if check_date.month == 1:
            s2, e2 = self.get_tet_window(check_date.year - 1, days_before)
            if s2 <= check_date <= e2:
                return True
        return start <= check_date <= end

    def get_tet_trading_days_before(self, year: int, n: int = 15) -> List[date]:
        tet = self.get_tet_date(year)
        if tet is None:
            return []
        result = []
        current = tet - timedelta(days=1)
        while len(result) < n:
            if current.weekday() < 5:
                yr_holidays = self.VN_HOLIDAYS.get(current.year, [])
