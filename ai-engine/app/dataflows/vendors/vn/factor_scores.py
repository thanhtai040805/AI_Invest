...
from app.dataflows.vendors.vn.sector_groups import (
    classify_major as classify,
    FINANCIALS, REAL_ESTATE, OTHERS,
)
from app.quant.factors.factor_orthogonalization import (
...
def compute_factor_scores(
    ...
    from app.dataflows.vendors.vn.news_events import compute_sentiment_5d
...
def refresh_all(
    ...
    try:
        ...
        logger.info("Optimized factor scores for date: %s", score_date)

        cur.execute("SELECT symbol FROM stocks WHERE exchange IN ('HOSE','HSX') ORDER BY symbol")
        symbols = [r[0] for r in cur.fetchall()]
        logger.info("Computing factor scores for %d symbols", len(symbols))
