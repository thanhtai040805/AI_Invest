...
Upserts into knowledge_documents (ON CONFLICT symbol+url SKIP).
"""
...
def _is_duplicate_news(title: str) -> bool:
    """Check if the news title indicates an official report that is already fetched as PDF."""
    t_lower = title.lower()
    dup_keywords = (
        "nghị quyết",
        "biên bản",
    ...
    return any(kw in t_lower for kw in dup_keywords)
...
def _parse_dulieu_page(
    ...
    for ul in tree.css("ul"):
        for li in ul.css("li"):
            ...
            if symbol and _is_duplicate_news(title):
                continue
            items.append({
...
def upsert_news(items: List[Dict[str, Any]]) -> Dict[str, int]:
    """Bulk upsert into knowledge_documents. Returns {inserted, skipped}."""
    if not items:
            ...
            for item in items:
                cur.execute(
                    """INSERT INTO knowledge_documents
                       (symbol, published_date, title, url, source, doc_type, sentiment_score)
                       VALUES (%s, %s, %s, %s, %s, %s, %s)
                       ON CONFLICT (symbol, url) DO UPDATE SET doc_type = EXCLUDED.doc_type""",
                    (
                        item["symbol"],
                        item["published_date"],
                        item["title"],
