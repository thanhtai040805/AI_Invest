...
class OHLCVIngestionService:
    ...
    def fetch_ohlcv(
        ...
        if symbol.upper() == 'VNINDEX':
            return self._fetch_from_dnse(symbol, start_date, end_date, bar_type="INDEX")
            
        # CHỈ lấy từ DNSE (Primary) - Theo mandate: Không dùng fallback data
        data = self._fetch_from_dnse(symbol, start_date, end_date)
        return data

    def _fetch_from_dnse(
        self, symbol: str, start_date: date, end_date: date, bar_type: str = "STOCK"
    ) -> List[Dict[str, Any]]:
