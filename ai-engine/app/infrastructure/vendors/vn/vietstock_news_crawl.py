...
CLI: python -m app.infrastructure.vendors.vn.vietstock_news_crawl [--channels 144,733] [--no-deep]
"""
...
from app.infrastructure.vendors.vn.news_utils import (
    extract_article_data, batch_has_content, upsert_articles as upsert,
    async_download_pdf_text,
)
...
async def fetch_channel_articles(client: httpx.AsyncClient, channel_id: int,
    ...
    for art in data:
        ...
        if not title:
            continue
        articles.append({"url": full_url, "title": title, "symbol": extract_symbol_from_url(url)})
...
if __name__ == "__main__":
    ...
    while i < len(args):
        if args[i] == "--channels" and i + 1 < len(args):
            ...
            print(f"Usage: python -m app.infrastructure.vendors.vn.vietstock_news_crawl [--channels 144,733] [--no-deep]")
            sys.exit(1)
