...
def compute_full_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Price Limits & Streaks (Fix 3)
    # HOSE limit is ±7%, using 6.9% as buffer
    prev_close = close.shift(1)
    out["at_ceiling"] = close >= prev_close * 1.069
    out["at_floor"] = close <= prev_close * 0.931

    ceil_series = out["at_ceiling"].astype(int)
    ceil_block = (ceil_series != ceil_series.shift(1)).cumsum()
    out["ceiling_streak"] = ceil_series.groupby(ceil_block).cumsum()

    floor_series = out["at_floor"].astype(int)
    floor_block = (floor_series != floor_series.shift(1)).cumsum()
    out["floor_streak"] = floor_series.groupby(floor_block).cumsum()
