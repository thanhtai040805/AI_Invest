...
if TYPE_CHECKING:
    from app.brain.config.schema import AgentConfig
...
def _discover_subclasses() -> list[type[BaseTool]]:
    ...
    for _, module_name, _ in pkgutil.iter_modules([pkg_dir]):
        if module_name.startswith("_"):
            continue
        try:
            importlib.import_module(f"app.brain.tools.{module_name}")
...
def build_registry(
    *,
    ...
    """
    from app.brain.tools.remember_tool import RememberTool
    from app.brain.tools.swarm_tool import SwarmTool
    ...
    if agent_config and agent_config.mcp_servers:
        from app.brain.tools.mcp import build_mcp_tool_wrappers, resolve_mcp_server_tool_name_segments
