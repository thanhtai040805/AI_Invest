"""
Evaluate Regime-Gated Integrated Alpha System on HOSE (2020-2026, 100% Spot Equity).
Target: Scientifically verify Realized Win Rate >= 70% - 75% when combining:
1. Macro Regime Gate (VN-Index >= MA50 / Non-Crisis, 100% Cash during Bear)
2. Graph Contagion 8D Lead-Lag Alpha
3. LambdaMART Cross-Sectional Ranker
4. Conformal Selective Sniper Gate (Z >= 2.85)
5. Asymmetric Dynamic Trailing Stop (Breakeven Lock + Hard Stop)
"""
import os
import sys
import logging
...
def simulate_regime_gated_trades(data_dict):
    # 1. Compute VNINDEX Regime Filter (MA50 trend)
    vnindex_df = data_dict.get('VNINDEX')
    if vnindex_df is not None:
        vnindex_df['ma50'] = vnindex_df['close'].rolling(50).mean()
        vnindex_df['regime_bullish'] = vnindex_df['close'] >= vnindex_df['ma50']
        regime_map = vnindex_df['regime_bullish'].to_dict()
    else:
        regime_map = {}
        
    logger.info("Generating Base Features for N=100...")
    base_features_dict = {}
    ...
    for ty in test_years:
        ...
        for dt, day_df in test_df.groupby(test_df.index):
            # Check Regime Gate
            is_bullish = regime_map.get(dt, True)
            ...
            for rank_idx, cand in enumerate(liquid_df.head(5).itertuples(), start=1):
                ...
                # Dynamic Asymmetric Trailing Stop:
                stopped = False
...
def analyze_and_print_results(tdf):
    print("\n" + "="*95)
    print(" REGIME-GATED INTEGRATED SYSTEM (N=100 HOSE, 2020 - 2026)")
    print("="*95)
    
    # 1. Compare All Days vs Regime Bullish Days
    print("\n--- 1. IMPACT OF MACRO REGIME GATE (VN-Index >= MA50 Filter) ---")
    
    all_days_top1 = tdf[tdf['rank'] == 1]
    bull_days_top1 = tdf[(tdf['rank'] == 1) & (tdf['regime_bullish'] == True)]
    
    print(f"All Days Top 1 Trades: {len(all_days_top1):4d} | Win Rate: {all_days_top1['is_win_managed'].mean()*100:.2f}% | Avg Ret: {all_days_top1['managed_return'].mean()*100:+.2f}%")
    print(f"Regime-Gated Top 1   : {len(bull_days_top1):4d} | Win Rate: {bull_days_top1['is_win_managed'].mean()*100:.2f}% | Avg Ret: {bull_days_top1['managed_return'].mean()*100:+.2f}%")
    
    # 2. Conformal Sniper Frontier under Regime Gate (Top 1)
    print("\n--- 2. CONFORMAL SNIPER FRONTIER UNDER REGIME GATE (TOP 1) ---")
    ...
    for z_th in [0.0, 2.0, 2.5, 2.85, 3.0, 3.2, 3.4, 3.6, 3.8]:
        sub_z = tdf[(tdf['rank'] == 1) & (tdf['regime_bullish'] == True) & (tdf['z_score'] >= z_th)]
        if len(sub_z) == 0:
            continue
    ...
    # 3. Top 3 Candidates under Regime Gate
    print("\n--- 3. CONFORMAL SNIPER FRONTIER UNDER REGIME GATE (TOP 3) ---")
    print(f"{'Z-Score Threshold':<20} | {'Trades':<8} | {'Pure Alpha WR':<14} | {'MANAGED WIN RATE':<18} | {'Avg Ret/Trade':<14} | {'Payoff'}")
    print("-" * 95)
    for z_th in [0.0, 2.0, 2.5, 2.85, 3.0, 3.2, 3.4, 3.6, 3.8]:
        sub_z = tdf[(tdf['rank'] <= 3) & (tdf['regime_bullish'] == True) & (tdf['z_score'] >= z_th)]
        if len(sub_z) == 0:
            continue
        alpha_wr = sub_z['is_win_alpha'].mean() * 100
        mng_wr = sub_z['is_win_managed'].mean() * 100
...
if __name__ == "__main__":
    ...
    tdf = simulate_regime_gated_trades(data_dict)
