...
export default function Compare() {
  ...
  useEffect(() => {
    ...
    }).catch((e: unknown) => console.error('Failed to list runs', e));
  }, []);
  ...
  useEffect(() => {
    if (leftId) {
      ...
      }).catch((e: unknown) => { console.error('Failed to load left run', e); setLeftData(null); setLeftCurve([]); });
    } else {
      setLeftData(null);
      setLeftCurve([]);
    }
  ...
  useEffect(() => {
    if (rightId) {
      ...
      }).catch((e: unknown) => { console.error('Failed to load right run', e); setRightData(null); setRightCurve([]); });
    } else {
      setRightData(null);
      setRightCurve([]);
    }
