...
import { CandlestickChart } from "@/components/vibe/charts/CandlestickChart";
import { EquityChart } from "@/components/vibe/charts/EquityChart";
import { MetricsCard } from "@/components/vibe/chat/MetricsCard";
import { ValidationPanel } from "@/components/vibe/charts/ValidationPanel";
...
function SkeletonChart({ height }: { height?: number }) {
  return (
    <div className="rounded-md border bg-card p-3">
      ...
      <Skeleton className="w-full rounded-md" style={{ height: height ?? 400 }} />
    </div>
  );
}
...
export default function RunDetail() {
  ...
  useEffect(() => {
    if (!runId) return;
    Promise.all([
      api.getRun(runId as string).catch((e: unknown) => { console.error('Failed to load run', e); return null; }),
      api.getRunCode(runId as string).catch((e: unknown) => { console.error('Failed to load run code', e); return {}; }),
    ...
    <div className="p-4 space-y-4">
      {entries.map(([sym, bars]) => (
        <div key={sym}>
          ...
          <CandlestickChart data={bars} markers={run.trade_markers?.filter((m) => (m as unknown as Record<string, unknown>).code === sym)} indicators={run.indicator_series?.[sym]} height={500} />
        </div>
      ))}
      {hasEquity && (
        <div>
