...
export default function Settings() {
  ...
  const providers = useMemo(() => settings?.providers ?? [], [settings?.providers]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setApiKey("");
      setClearApiKey(false);
      setSuccessMessage(t.llmSettingsSaved);
      setTimeout(() => setSuccessMessage(null), 3000);
    } catch (error) {
      setSuccessMessage(`${t.llmSettingsSaveFailed}: ${error instanceof Error ? error.message : t.unknownError}`);
  ...
  const submitDataSources = async (event: FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage(`${t.dataSourceSettingsSaveFailed}: ${error instanceof Error ? error.message : t.unknownError}`);
      setTimeout(() => setSuccessMessage(null), 5000);
    } finally {
      setDataSaving(false);
    }
